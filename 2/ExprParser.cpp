...
void ExprParser::DoUnaryOperation(const char op)
{
	...
	m_infixExprStack.Push({ expr, priority, rightAssoc, true });
}
...
void ExprParser::DoBinaryOperation(const char op)
{
	...
	m_calcStack.Push(result);
	m_infixExprStack.Push({ leftExpr + op + rightExpr, priority, rightAssoc, left.isFirstUnary });
}
...
std::string ExprParser::WrapParensIfNeed(const InfixNode& node, const uint8_t parentPriority, const bool isRightOperand, const bool parentRightAssoc, const bool parentCommutare)
{
	...
	else if (node.isFirstUnary && ((isRightOperand && !parentRightAssoc) || (!isRightOperand && parentRightAssoc)))
	{
		needParens = true;
	}
	//std::cout << std::to_string(parentPriority) << " " << std::to_string(node.isFirstUnary) << std::endl;
